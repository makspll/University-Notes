#! /usr/bin/env python
#
# Version 0.9  (HS 09/03/2020)
#
import numpy as np
import scipy.io
import matplotlib.pyplot as plt
import task2_hNN_A as t2


# no of points is the square of the width of the graph
width = 1000

#calculate mesh
xs = np.linspace(0,3,num=width)
ys = np.linspace(0,3,num=width)

pointsX,pointsY = len(xs),len(ys)

X,Y = np.meshgrid(xs,ys)

# 2-by-width-by-width
coordinate_grid = np.stack((X.ravel(), Y.ravel()),axis=1)
print(coordinate_grid)
Z = t2.task2_hNN_A(coordinate_grid).reshape(pointsX,pointsY)
fig,ax = plt.subplots(1)
plt.xlim(0,3)
plt.ylim(0,3)
plt.gca().set_aspect('equal',adjustable='box')
ax.contourf(X,Y,Z,levels=1,colors=['w','r'])

ax.set_xlabel("x")
ax.set_ylabel("y")
ax.set_title("Decision Boundaries with hneurons")
plt.show()