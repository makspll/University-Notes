...
{
    while(1) {
    //check for out of bounds either way, wrap around 
    if(string >= grid + grid_total_length || *string == '\n')
    {
      int x = string_idx % grid_row_length;
      int y = string_idx / grid_row_length;
      //we go diagonally left and up, by min(x,y) steps to get back within bounds and then to the other side of the grid
      int steps = x;
      if(x > y){steps = y;} 
      string -= (steps) * (grid_row_length + 1);
      string_idx -= (steps) * (grid_row_length + 1);
      //i tried using offset pointer - base pointer to calculate idx, but it would be machine dependent because of pointer sizes i think,
      //so just decided to pass in the grid_idx
    }
