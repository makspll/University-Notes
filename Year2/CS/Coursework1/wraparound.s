...
CONTAIN_D_LOOP:			#while(1) {
	...
	lw $t0, grid_total_length
	la $t1, grid
	addu $t0,$t0,$t1 # t0 <- grid_total_length + grid
	
	sge $t0, $a2, $t0	#t0 = 1 if string >= grid+grid_total_length
	beq $t0, 1, CONTAIN_D_IF1_ENTRY	#if(string >= grid + grid_total_length ..
	
	lbu $t0, ($a2)
	bne $t0, 10, CONTAIN_D_IF1_EXIT	 #..|| *string == '\n')
	
CONTAIN_D_IF1_ENTRY:
	lw $t5, grid_row_length
	div $a3, $t5
	mfhi $t0		#t0 <- x, the reminder       int x = string_idx % grid_row_length;
 	mflo $t1		#t1 <- y, the quotient       int y = string_idx / grid_row_length;
 	
 	addu $t2, $t0, $zero	# int steps = x;
	ble $t0, $t1 CONTAIN_D_IF2_EXIT # if(x > y)
	addu $t2, $t1, $zero	# steps = y;
CONTAIN_D_IF2_EXIT:
	addiu $t5, $t5, 1	#  t5 <- grid_row_length + 1
	multu $t5, $t2		#  (steps) * (grid_row_length + 1);
	mflo $t0		#  t0<-/ we're truncating top 32 bits, we won't ever need that
	subu $a2, $a2, $t0	#  string -= (steps) * (grid_row_length + 1);
	subu $a3, $a3, $t0	#  string_idx -= (steps) * (grid_row_length + 1);
	
CONTAIN_D_IF1_EXIT:

	lbu $t0, ($a2)		#to <- *string
	lbu $t1, ($a1)		#t1 <- *word
	beq $t0,$t1 CONTAIN_D_IF3_EXIT # if ( *string!= *word)
